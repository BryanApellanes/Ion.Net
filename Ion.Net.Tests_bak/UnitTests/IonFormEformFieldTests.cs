    ...
    ""eform"": {
      ...
      ""value"": [
        ...
        { ""name"": ""password"", ""secret"": true }
            ...
            IonForm.Validate(formJson).Should().Be(false); // verify that the form is not valid.  form is the same as form in form field
            IonFormField ionFormField = IonFormField.Read(formFieldJson);
            ionFormField["eforma"].Should().BeNull(); // should be null because not a valid form (duplicate username fields)
        }
        /* 
